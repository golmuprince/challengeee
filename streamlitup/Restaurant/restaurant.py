...
st.set_page_config(
    page_title="Golmu 레스토랑 Assistant",
    page_icon="🍽️",
    layout="centered"
)
st.title("Golmu 레스토랑 Assistant")
st.caption("메뉴 확인, 예약, 주문을 도와드려요!")
...
async def run_agent(message):
    ...
    with st.chat_message("ai"):
        ...
        try:
            ...
            st.write(f"InputGuardrail 발동")
            st.write("저는 레스토랑 관련 질문에 대해서만 도와드리고 있어요. 메뉴를 확인하거나, 예약하거나, 음식을 주문할 수 있어요.")
        
        except OutputGuardrailTripwireTriggered:
            st.write(f"OutputGuardrail 발동")
            st.write("이 메세지는 우리 정책과 맞지 않아요.")
