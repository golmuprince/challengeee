...
from models import MessageOutputGuardRailOutput, UserAccountContext

Mesaage_output_guardrail_agent = Agent(
...
""",
    output_type=MessageOutputGuardRailOutput,
)

@output_guardrail
async def Message_Output_guardrail(
    wrapper:  RunContextWrapper[UserAccountContext],
    agent: Agent,
    output:str
):
    result = await Runner.run(
        Mesaage_output_guardrail_agent,
        output,
    ...
    or validation.contains_leaks_internal_info
    or validation.contains_hallucination
    or validation.contains_violates_privacy
)
